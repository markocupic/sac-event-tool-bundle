...
class tl_calendar_events_sac_event_tool extends tl_calendar_events
{
        ...
        if (Input::get('act') === 'edit')
        {
            ...
            if ($objCalendarEventsModel->eventState === 'event_deferred')
            {
                \Contao\CoreBundle\DataContainer\PaletteManipulator::create()
                    ->addField(['eventDeferDate'], 'eventState', \Contao\CoreBundle\DataContainer\PaletteManipulator::POSITION_AFTER)
    ...
    public function onloadCallback(DataContainer $dc)
    {
        ...
        if (Input::get('act') === 'editAll' || Input::get('act') === 'overrideAll')
        {
            ...
            {
                \Contao\CoreBundle\DataContainer\PaletteManipulator::create()
                    ->addField(['eventReleaseLevel'], 'title_legend', \Contao\CoreBundle\DataContainer\PaletteManipulator::POSITION_APPEND)
                    ->applyToPalette('default', 'tl_calendar_events');
            }
        }
            ...
            if ($objEventsModel !== null)
            {
                if (EventReleaseLevelPolicyModel::findByPk($objEventsModel->eventReleaseLevel) !== null)
                {
                    if (!EventReleaseLevelPolicyModel::hasWritePermission($this->User->id, $objEventsModel->id) && $this->User->id !== $objEventsModel->registrationGoesTo)
                    {
                        ...
                        {
                            $GLOBALS['TL_DCA']['tl_calendar_events']['fields'][$field]['input_field_callback'] = ['tl_calendar_events_sac_event_tool', 'showFieldValue'];
                        }
                                    ...
                                    if ($objEventReleaseLevelPolicyModel !== null)
                                    {
                                        if ($objEventReleaseLevelPolicyModel->id != $objEventsModel->eventReleaseLevel)
                                        {
                                            foreach ($GLOBALS['TL_DCA']['tl_calendar_events']['fields'] as $fieldname => $arrDca)
                                            {
                                                ...
                                                {
                                                    $GLOBALS['TL_DCA']['tl_calendar_events']['fields'][$fieldname]['input_field_callback'] = ['tl_calendar_events_sac_event_tool', 'showFieldValue'];
                                                }
                                            }
                                        }
                                    }
            ...
            if ($objEvent !== null)
            {
                while ($objEvent->next())
                {
                    ...
                    foreach ($arrFields as $field)
                    {
                        ...
                        {
                            $arrOrganizers = \Markocupic\SacEventToolBundle\CalendarEventsHelper::getEventOrganizersAsArray($objEvent->current(), 'title');
    ...
    public function showFieldValue(Contao\DC_Table $dc)
    {
        ...
        foreach ($fields as $i)
        {
            ...
            elseif ($GLOBALS['TL_DCA'][$strTable]['fields'][$i]['eval']['rgxp'] == 'datim' || in_array($GLOBALS['TL_DCA'][$strTable]['fields'][$i]['flag'], [5, 6, 7, 8, 9, 10]) || $i == 'tstamp')
            {
