        ...
        if ($strTemplate === 'be_main')
        {
            ...
            if (Input::get('do') === 'sac_calendar_events_tool' && Input::get('table') === 'tl_calendar_events_member')
            {
                if (preg_match('/<table class=\"tl_listing(.*)<\/table>/sU', $strBuffer))
                {
                    ...
                    {
                        $strLegend .= sprintf('<li><img src="%s/icons/%s.svg" width="16" height="16"> %s</li>', SACP_ASSETS_DIR, $state, $GLOBALS['TL_LANG']['tl_calendar_events_member'][$state]);
                    }
                    $strLegend .= '</ul>';
                    $strLegend .= '</div>';
                    ...
                    $strLegend .= '<ul>';
                    $strLegend .= sprintf('<li><img src="%s/icons/%s.svg" width="16" height="16"> %s</li>', SACP_ASSETS_DIR, 'has-not-participated', 'Hat am Event nicht/noch nicht teilgenommen');
                    $strLegend .= sprintf('<li><img src="%s/icons/%s.svg" width="16" height="16"> %s</li>', SACP_ASSETS_DIR, 'has-participated', 'Hat am Event teilgenommen');
                    $strLegend .= '</ul>';
                    $strLegend .= '</div>';
